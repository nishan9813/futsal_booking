    ...
    <div className="owner-form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <section className="mb-6">
          <h2 className="text-lg font-semibold mb-4">Futsal Info</h2>
          <input type="text" name="futsal_name" placeholder="Futsal Name" value={ownerData.futsal_name} onChange={handleOwnerChange} />
          {errors.futsal_name && <p className="text-red-500 text-sm">{errors.futsal_name}</p>}
          <input type="text" name="address" placeholder="Address" value={ownerData.address} onChange={handleOwnerChange} />
          {errors.address && <p className="text-red-500 text-sm">{errors.address}</p>}
          <div className="grid grid-cols-2 gap-4">
            <input type="text" name="city" placeholder="City" value={ownerData.city} onChange={handleOwnerChange} />
            <input type="text" name="country" placeholder="Country" value={ownerData.country} onChange={handleOwnerChange} />
          </div>
          {errors.city && <p className="text-red-500 text-sm">{errors.city}</p>}

          <div className="mt-4">
            <label className="block mb-2">Location</label>
            {!ownerData.location && <p className="text-sm text-gray-600 mb-2">No location selected yet.</p>}
            {ownerData.location && <p className="text-sm text-green-700 mb-2">Lat {ownerData.location.lat.toFixed(4)}, Lng {ownerData.location.lng.toFixed(4)}</p>}
            <button type="button" onClick={() => setShowMap(true)} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Select Location</button>
            {errors.location && <p className="text-red-500 text-sm mt-1">{errors.location}</p>}
          </div>
        </section>
        ...
        <section className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">Grounds</h2>
            <button type="button" className="text-blue-600" onClick={() => setGrounds(prev => [...prev, { ground_type: "5A", opening_time: "06:00", closing_time: "22:00", price: 2000, images: [] }])}>+ Add Ground</button>
          </div>
          {grounds.map((ground, index) => (
            <div key={index} className="ground-card">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-medium">Ground #{index + 1}</h3>
                {grounds.length > 1 && <button type="button" className="text-red-600" onClick={() => setGrounds(prev => prev.filter((_, i) => i !== index))}>Remove</button>}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                <select name="ground_type" value={ground.ground_type} onChange={(e) => handleGroundChange(index, e)}>
                  <option value="5A">5-a-side</option>
                  <option value="7A">7-a-side</option>
                  <option value="MAT">Mat Futsal</option>
                  <option value="TURF">Turf</option>
                ...
                <input type="number" name="price" placeholder="Price" value={ground.price} onChange={(e) => handleGroundChange(index, e)} />
                <input type="time" name="opening_time" value={ground.opening_time} onChange={(e) => handleGroundChange(index, e)} />
                <input type="time" name="closing_time" value={ground.closing_time} onChange={(e) => handleGroundChange(index, e)} />
              </div>
